...
import { XMatchedPathRequestAdapter } from './x-matched-path-request-adapter'
...
describe('XMatchedPathRequestAdapter', () => {
  describe('with ppr', () => {
    const adapter = new XMatchedPathRequestAdapter(
      '',
    ...
    it('should not detect rsc if it is not present in the url', async () => {
      ...
      expect(parsedURL.query.__nextDataReq).toBeUndefined()
    })
    ...
    it('should detect rsc requests', async () => {
      ...
      expect(req.headers['next-router-prefetch']).toBeUndefined()
      expect(parsedURL.query.__nextDataReq).toBe('1')
    })
